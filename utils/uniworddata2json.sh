...
Options:
  ...
  --arpa-lm-file <arpa-lm-file>                    # this is an arpa-formatted LM file
...
opts=""
if [ -n "${feat}" ]; then
    intypes="${input} word output other"
else
    intypes="word output other"
fi
...
probword_merge_scp2json.py --verbose ${verbose} ${opts}
