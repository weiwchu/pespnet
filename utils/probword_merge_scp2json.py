...
def get_parser():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="The json files except for the input, words and outputs",
    )
...
if __name__ == "__main__":
    ...
    word_fscps = [
        [open(i[1], "r", encoding="utf-8") for i in il] for il in word_infos
    ]
    output_fscps = [
    ...
    while True:
        ...
        word_lines = [[f.readline() for f in fl] for fl in word_fscps]
        output_lines = [[f.readline() for f in fl] for fl in output_fscps]
        lines = [[f.readline() for f in fl] for fl in fscps]

        # Get the first line
        concat = sum(input_lines + word_lines + output_lines + lines, [])
        if len(concat) == 0:
            break
        first = concat[0]
        ...
        count = 0
        for ls_list in (input_lines, word_lines, output_lines, lines):
            for ls in ls_list:
                for line in ls:
                    if line == "" or first == "":
                        if line != first:
                            concat = sum(input_infos + word_lines + output_infos + infos, [])
                            raise RuntimeError(
                        ...
                        concat = sum(input_infos + word_infos + output_infos + infos, [])
                        raise RuntimeError(
