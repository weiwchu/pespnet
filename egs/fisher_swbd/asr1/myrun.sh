...
# data
# /nws/speech/fromNAS5/ldc/fisher/
fisher_dir="/ndata/speech/engl1/spon/fisher"
swbd1_dir=/ndata/speech/engl1/spon/swbd
eval2000_dir="/ndata/speech/ldc/hub5e_00 /ndata/speech/ldc/2000_hub5_eng_eval_tr"
...
train_set=train_nodev     # 4000 of swbd
train_dev=train_dev       # fisher + swbd wo train nodev
train_all=train_all
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    # don't use rt03, use only part of swbd for dev
    utils/subset_data_dir.sh --first data/train_swbd 4000 data/train_dev_untrim # 5hr 6min
    n=$[`cat data/train_swbd/segments | wc -l` - 4000]
    utils/subset_data_dir.sh --last data/train_swbd $n data/train_swbd_nodev_untrim  # ~280 hr

    utils/combine_data.sh data/train_all data/train_fisher data/train_swbd    # everything
    utils/combine_data.sh data/train_nodev_untrim data/train_fisher data/train_swbd_nodev_untrim
    # vs data/train_dev
    ...
    for x in train_all train_nodev_untrim train_dev_untrim eval2000; do
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in train_nodev_untrim train_dev_untrim ${recog_set}; do
    ...
    remove_longshortdata.sh --maxframes 3000 --maxchars 400 data/train_dev_untrim data/${train_set}
    remove_longshortdata.sh --maxframes 3000 --maxchars 400 data/train_nodev_untrim data/${train_dev}
...
dict=data/lang_1char/${train_all}_units.txt
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    cut -f 2- data/${train_all}/text | tr " " "\n" | sort | uniq | grep "\[" > ${nlsyms}
    cat ${nlsyms}

    echo "make a dictionary"
    echo "<unk> 1" > ${dict} # <unk> must be 1, 0 will be used for "blank" in CTC
    text2token.py -s 1 -n 1 -l ${nlsyms} data/${train_all}/text | cut -f 2- -d" " | tr " " "\n" \
